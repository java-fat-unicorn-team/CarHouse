...
public class CarSaleDaoImpl implements CarSaleDao {
     ...
     * Throw exception if carSale hasn't been deleted
     *
     * @param carSaleId the car sale id
     */
    @Override
    public void deleteCarSale(final int carSaleId) {
        ...
        SqlParameterSource parameters = new MapSqlParameterSource()
                .addValue("id", carSaleId);
        if (namedParameterJdbcTemplate.update(DELETE_CAR_SALE_SQL, parameters) != 1) {
            throw new EmptyResultDataAccessException(carSaleId);
        }
    }
     ...
     */
    @Override
    public String getCarSaleImageName(final int carSaleId) {
        SqlParameterSource parameters = new MapSqlParameterSource()
                .addValue("id", carSaleId);
        return namedParameterJdbcTemplate.queryForObject(GET_CAR_SALE_IMAGE_NAME_SQL,
